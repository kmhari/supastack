...
echo "[db-push] step 2/7: supabase db push (no --db-url)"
SUPABASE_ACCESS_TOKEN="$SELFBASE_PAT" \
...
echo "[db-push] step 3/7: supabase migration list"
...
# --- 6. supabase db diff (should show no pending changes) ---------------
echo "[db-push] step 4/7: supabase db diff"
SUPABASE_ACCESS_TOKEN="$SELFBASE_PAT" \
  supabase --profile "$WORK/selfbase.toml" db diff \
    --project-ref "$SELFBASE_PROJECT_REF" >/dev/null
echo "[db-push] ✓ db diff exit 0"

# --- 7. supabase db pull (schema dump) ----------------------------------
echo "[db-push] step 5/7: supabase db pull"
SUPABASE_ACCESS_TOKEN="$SELFBASE_PAT" \
...
echo "[db-push] step 6/7: supabase inspect db"
SUPABASE_ACCESS_TOKEN="$SELFBASE_PAT" \
...
echo "[db-push] step 7/7: cleanup (drop _e2e_db_push_test via psql)"
