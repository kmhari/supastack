...
create_org() { # name -> echoes id
  curl -sS -X POST "${AUTH[@]}" -d "{\"name\":\"$1\"}" "${BASE}/platform/organizations" \
...
# Rename org1.
ok "rename-200" 200 "$(curl -sS -o /dev/null -w '%{http_code}' -X PATCH "${AUTH[@]}" -d '{"name":"E2E Renamed"}' "${BASE}/platform/organizations/${ID1}")"
curl -sS "${AUTH[@]}" "${BASE}/platform/organizations/${ID1}" | grep -q '"name":"E2E Renamed"' \
  && ok "rename-applied" yes yes || ok "rename-applied" yes no

# Org appears in the list.
curl -sS "${AUTH[@]}" "${BASE}/platform/organizations" | grep -q "$ID2" \
...
# a real project here.
ok "delete-empty-org1-204" 204 "$(curl -sS -o /dev/null -w '%{http_code}' -X DELETE "${AUTH[@]}" "${BASE}/platform/organizations/${ID1}")"
ok "delete-empty-org2-204" 204 "$(curl -sS -o /dev/null -w '%{http_code}' -X DELETE "${AUTH[@]}" "${BASE}/platform/organizations/${ID2}")"
