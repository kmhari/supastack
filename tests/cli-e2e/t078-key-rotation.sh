...
#   SUPASTACK_TEST_PROJECT_REF='<project ref>' \
#   DATABASE_URL='postgres://...' \
#   bash tests/cli-e2e/t078-key-rotation.sh
#
# Optional:
#   SUPASTACK_VM_USER=ubuntu  (default: ubuntu)
#
# Requirements: curl, jq, openssl, ssh (key-based auth to SUPASTACK_VM_USER@SUPASTACK_APEX)
# Duration: ~5 minutes
...
: "${SUPASTACK_PAT:?SUPASTACK_PAT required — operator Personal Access Token}"
: "${SUPASTACK_TEST_PROJECT_REF:?SUPASTACK_TEST_PROJECT_REF required — ref of a running project}"
: "${DATABASE_URL:?DATABASE_URL required — control-plane Postgres connection string}"
SUPASTACK_VM_USER="${SUPASTACK_VM_USER:-ubuntu}"

API="https://api.${SUPASTACK_APEX}"
VM="${SUPASTACK_VM_USER}@${SUPASTACK_APEX}"
RUN_START=$(date +%s)
...
if [[ -n "${OLD_MASTER_KEY:-}" ]]; then
  ...
  OLD_KEY=$(ssh "$VM" "sudo docker exec supastack-api-1 printenv MASTER_KEY" 2>/dev/null)
fi
...
ssh "$VM" "sudo docker cp /opt/supastack/scripts/rekey-master.mjs supastack-api-1:/app/packages/db/rekey-master.mjs" 2>/dev/null
DRYRUN_OUT=$(ssh "$VM" \
  "sudo docker exec \
     -e DRY_RUN=1 \
     ...
     supastack-api-1 node /app/packages/db/rekey-master.mjs" 2>&1)
...
EOF
chmod 600 "$TMPENV"
sudo docker compose -f /opt/supastack/infra/docker-compose.yml --env-file "$TMPENV" \
...
APIKEYS_RES=$(curl -sk "${API}/v1/projects/${SUPASTACK_TEST_PROJECT_REF}/api-keys" \
  -H "Authorization: Bearer ${SUPASTACK_PAT}")
...
curl -sk -X POST "${API}/v1/projects/${SUPASTACK_TEST_PROJECT_REF}/pause" \
  -H "Authorization: Bearer ${SUPASTACK_PAT}" -o /dev/null
for i in $(seq 1 12); do
  STATUS=$(curl -sk "${API}/v1/projects/${SUPASTACK_TEST_PROJECT_REF}" \
    -H "Authorization: Bearer ${SUPASTACK_PAT}" | jq -r '.status')
...
curl -sk -X POST "${API}/v1/projects/${SUPASTACK_TEST_PROJECT_REF}/restore" \
  -H "Authorization: Bearer ${SUPASTACK_PAT}" -o /dev/null
...
# ── Step 10: Kong health ──────────────────────────────────────────────────────
KONG_URL=$(curl -sk "${API}/v1/projects/${SUPASTACK_TEST_PROJECT_REF}" \
  -H "Authorization: Bearer ${SUPASTACK_PAT}" | jq -r '.endpoint // empty')
if [[ -n "$KONG_URL" ]]; then
...
echo
echo "[T078] PASS: master-key rotation validated | total_elapsed: ${TOTAL_ELAPSED}s | rotated_at: ${ROTATED_AT} | project: ${SUPASTACK_TEST_PROJECT_REF}"
