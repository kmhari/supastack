/**
 * Shared zsh/bash wrapper snippet for supastack CLI integration.
 * Used in both /settings/cli and the setup wizard Step 4.
 *
 * Updated for the single `.supastack` file format (feature 024):
 *   token=sbp_xxx
 *   domain=supaviser.dev
 *
 * Replaces the old two-file system (.supastack for domain + .supabase_token for token).
 */
export function getWrapperSnippet(_apex: string): string {
  return `# supastack + supabase wrapper:
...
export function getSupastackFileContent(token: string, domain: string): string {
  return `token=${token}\ndomain=${domain}`;
}
