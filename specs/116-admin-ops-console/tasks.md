...
- [ ] T004 Idempotent + additive migration `packages/db/migrations/0022_observability.sql` creating `resource_samples` (time-series) + `control_plane_snapshots` (latest-per-container) using `GENERATED BY DEFAULT AS IDENTITY` (NOT `SEQUENCE … OWNED BY`), plus Drizzle schema `packages/db/src/schema/observability.ts` and its export in `packages/db/src/schema/index.ts`
- [ ] T005 [P] Secret-redaction helper `redactSensitive(text)` masking `postgres://…`, `sbp_…`, `Bearer …`, `password=…` in `apps/api/src/services/job-redactor.ts` with happy + sad unit tests (masks each pattern; leaves clean text intact) in `apps/api/tests/unit/job-redactor.test.ts`
- [ ] T006 Worker `observer` repeatable job in `apps/worker/src/jobs/observer.ts` — `docker stats --no-stream` (aggregate per `supastack-<ref>-*` + host), `df`/`du` on `/var/supastack/{instances,backups}` (disk breakdown), `docker inspect` health + image for control-plane containers, `docker logs --tail N` (redacted) → upsert `control_plane_snapshots`, insert `resource_samples`, prune older than `OBSERVER_RETENTION_DAYS`; register at boot in `apps/worker/src/main.ts` with `repeat: { every: OBSERVER_INTERVAL_MS }` using `QUEUES.observer`
- [ ] T007 [P] Worker observer unit test (fake docker output → per-project aggregation, host disk breakdown, control-plane snapshot upsert, log-tail redaction, prune) in `apps/worker/tests/unit/observer.test.ts`
...
- [ ] T024 [US2] Admin `Fleet` + `ProjectDetail` pages in `apps/web/src/pages/admin/Fleet.tsx` and `apps/web/src/pages/admin/ProjectDetail.tsx`
...
- [ ] T030 [US3] Admin `Resources` page (host totals, per-project ranking, disk breakdown, avg footprint, sparkline trend, collecting state) in `apps/web/src/pages/admin/Resources.tsx`
...
- [ ] T041 Deploy + live-VM smoke per [quickstart.md](./quickstart.md): rebuild `web` + `api` + `worker`, run migration, reload Caddy; verify docs personalization, admin gate (admin 200 / non-admin 403), fleet/system/logs, resources after a tick, queues redaction, certs/DNS/backups
