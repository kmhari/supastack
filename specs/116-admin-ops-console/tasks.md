...
- [ ] T004 Idempotent + additive migration `packages/db/migrations/00NN_observability.sql` — **use the next-free migration number** (verify by listing `packages/db/migrations/`; `0021` is the current latest, so likely `0022`) — creating `resource_samples` (time-series) + `control_plane_snapshots` (latest-per-container) using `GENERATED BY DEFAULT AS IDENTITY` (NOT `SEQUENCE … OWNED BY`), plus Drizzle schema `packages/db/src/schema/observability.ts` and its export in `packages/db/src/schema/index.ts`
...
- [ ] T007 [P] Worker observer unit test (fake docker output → per-project aggregation, host disk breakdown, control-plane snapshot upsert, log-tail redaction, prune) in `apps/worker/tests/unit/observer.test.ts`; also assert sampling is **one-shot / non-blocking** (invokes `docker stats --no-stream`, never a streaming/continuous call) and the tick completes within a bounded duration on the fixture (FR-020)
