...
- [ ] T029 [P] [US4] Add to `apps/api/src/services/backups-mgmt-service.ts` (or a new sibling `restore-service.ts`):
  - `initiateRestore(ref, callerUserId, payload: RestoreRequest): Promise<{ restore_job_id }>` — resolves backup_id from either field, runs preflight (RBAC admin-only, project not paused, backup status COMPLETED, blob present, PG-version match), wraps INSERT `restore_jobs` + UPDATE `supabase_instances.status='restoring'` in a single TX, enqueues the BullMQ `restore` job, emits audit log `mgmt_api.backup.restore_started`.
...
- [ ] T031 [P] [US4] Create `apps/worker/src/jobs/restore.ts` — BullMQ worker handling `{ restore_job_id }`. Implements the full state machine from `contracts/backups.md` "Worker job: restore" section:
  1. Load job, idempotency check.
  2. Set running.
  3. Fetch backup blob to `/tmp/restore-<job_id>.tar.gz` via the existing `@selfbase/backup-store` package.
  4. Take pre-restore snapshot (mv data → data.pre-restore-<id>); record path.
  5. `docker compose stop db` for the instance (via existing `@selfbase/docker-control` helpers).
  6. Extract blob into the empty `db/data` dir (tar -xzf).
  7. Start db service, wait healthy (5-min timeout).
  8. Smoke probe via `withPerInstancePg`.
  9. On success: status=`success`, UPDATE `supabase_instances.status='running'`, schedule delayed BullMQ message to GC pre-restore dir in 24h, audit `mgmt_api.backup.restore_completed`.
  10. On any failure: rollback (swap data dirs back, restart), status=`failed`, error_message populated, audit `mgmt_api.backup.restore_failed`.
- [ ] T032 [US4] Edit `apps/worker/src/index.ts` to register the restore queue + worker. Add a separate `restore-gc` worker that consumes the delayed messages from step 9 of T031 and removes the `pre-restore-<id>` directory + clears `restore_jobs.pre_restore_dir`.
- [ ] T033 [P] [US4] Create `tests/cli-e2e/backups-restore.sh` — Quickstart US4 round-trip. Takes a backup via the existing `/instances/:ref/backups` POST, makes a destructive change, runs `supabase backups restore`, polls `/restore-status` until success/failed, asserts final state matches the backup snapshot. Cleans up the test artifacts.
