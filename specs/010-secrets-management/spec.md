...
2. **Supastack dashboard UI for edge function secrets** — Studio's `/project/default/functions/secrets` page is broken (docs only, no CRUD). Supastack has no dashboard surface for secret management today. Operators `curl` or shell into containers. The dashboard needs a real edit page at `/dashboard/project/<ref>/secrets`, and Studio's broken URL needs to redirect there.
...
| Supastack dashboard (`/dashboard/project/<ref>/secrets`) | Operator-facing CRUD UI; writes call `vault.create_secret` / `vault.update_secret` / `delete from vault.secrets` via the api |
...
4. **Given** the operator tries to save a reserved name (`SUPABASE_URL`, `JWT_SECRET`, etc.), **When** they Save, **Then** the request is rejected with: "SUPABASE_URL is a reserved secret managed by supastack. Reserved names cannot be set." The whole batch is rejected atomically.
...
4. **Given** the extensions are enabled, **When** the operator visits Studio's Vault page, **Then** the page renders and secrets created via SQL or via supastack's dashboard (US1) appear; secrets created via Studio's UI are readable from SQL and from edge functions (US1).
...
6. **Given** the extensions are enabled, **When** the operator deletes their project via the existing lifecycle delete, **Then** the per-project Postgres is dropped normally — nothing in supastack's control plane references vault state.
...
**Independent Test**: With vault enabled and a `TEST_KEY=alpha` row in `vault.secrets`, invoke an edge function reading `Deno.env.get('TEST_KEY')` — must return `'alpha'`. Update the vault row to `'beta'`. Within at most one TTL window (≤10s) a fresh invocation must return `'beta'`. `docker logs supastack-<ref>-functions-1` must show zero restart events across the test.
...
3. **Given** an authenticated Studio session redirects, **When** the operator lands on supastack, **Then** the dashboard session cookie carries over (both Studio and dashboard sit under the same apex's cookie scope) and they don't bounce to `/login`.
...
- **Operator on supastack but not logged in**: redirect lands them at `/login` then bounces back to the secrets page after auth (existing dashboard pattern).
...
- **FR-004**: Studio's bundled Vault UI MUST render correctly for any per-project Studio once the extensions are enabled — no supastack-side modification to Studio.
...
- **FR-015**: The runtime MUST cache the decrypted vault map in-process with a TTL. Default TTL is **5 seconds**; configurable via the `SUPASTACK_VAULT_TTL_MS` env var on the functions container. Cache invalidation is passive — the cache is never busted from outside the runtime; the next read after the TTL elapses triggers a refresh. (No Redis pub/sub, no HTTP invalidate endpoint.)
...
- **Vault secret** (the single source of truth): row in per-project Postgres `vault.secrets` table (pgsodium-encrypted), readable via `vault.decrypted_secrets`. Owned and managed by per-project Postgres; supastack's api reads/writes it via SQL but stores no copies in the control plane.
- **Reserved secret name** (existing, now in `packages/shared`): the list of names supastack injects into containers at start (`SUPABASE_URL`, `JWT_SECRET`, etc.) — guarded at write time (rejected by the api) and at injection time (FR-014 reserved-env-wins guard).
...
- **SC-006**: Studio's bundled Vault UI renders without error for any project on the deployment after backfill, and secrets visible there match those visible in supastack's dashboard (US1) and to SQL callers — single source of truth. (US2)
- **SC-007**: For 100% of projects, clicking Studio's "Secrets" link in the Edge Functions sidebar lands the operator on the working supastack secrets page within 1 second. (US4)
...
- Studio's bundled Vault UI uses the same `vault.secrets` storage and therefore stays in sync with supastack's dashboard automatically — no integration glue between the two surfaces.
...
- Out of scope: rotating supastack's master key or re-encrypting old `project_secrets` blobs.
