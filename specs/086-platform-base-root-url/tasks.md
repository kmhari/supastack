...
**Organization**: by user story (US1 P1, US2 P2, US3 P2, US4 P3). `/api/v1` is the RETAINED internal engine — not deleted.
...
- [ ] T002 Resolve the unrelated uncommitted change in `apps/api/src/routes/platform-misc.ts` (billing stubs `free`→`pro` + `buildOrg` mocks) per operator decision (keep-on-separate-commit / stash / revert) BEFORE editing that file in US3, so 086's diff stays clean.
...
- [ ] T003 (no-op gate) Confirm no DB migration is introduced by this feature (Constitution I) and `/v1` OpenAPI snapshot is untouched (Constitution IV) — both must remain true through all phases.
...
- [ ] T018 [P] [US2] Trim `apps/web/tests/unit/api.test.ts` to assertions for the kept methods only (happy: correct URL/method/body; sad: e.g. a removed-group assertion deleted). Keep `setupApi`, `authApi.me`, `apexApi`, `orgApi.patch`, `wildcardCertApi.{initiate,verify,status}`.
...
- [ ] T021 [US2] Verify: `pnpm --filter @supastack/web build` succeeds (no dangling imports) and `pnpm --filter @supastack/web test` + `pnpm --filter @supastack/web lint` are green.

**Checkpoint**: apps/web is setup-only; builds + tests green.
...
- [ ] T023 [P] [US3] Add `apps/api/tests/unit/org-store.test.ts` — happy: inserts exactly one org + one `owner` membership, returns `{id,name}`, id matches ref format (mock `db`/`tx`). Sad: name handling is the caller's contract (assert the primitive forwards `name` verbatim; empty-name rejection is asserted at the route in T024).
- [ ] T024 [US3] Wire `POST /platform/organizations` (`apps/api/src/routes/platform-misc.ts:284-297`) to call `createOrganizationWithOwner` inside `db().transaction`; keep `requireAuth`, the empty-name 400, and the `buildOrg(...)` + `pending_payment_intent_secret` response shape unchanged.
- [ ] T025 [US3] Wire `apps/api/src/routes/setup.ts`: inside the existing transaction, after the in-tx `setup_state` re-check, replace the pre-tx `generateRef()` (line 48) + inline org/member inserts (67-77) with `const { id: orgId } = await createOrganizationWithOwner(tx, { userId: operator.id, name: body.orgName })`. Keep installation insert, `setup_state`, audit (`targetId: orgId`), `mintApiToken`, and the ownerless-org backfill.
- [ ] T026 [US3] Verify first-user creation is GoTrue-only: confirm `setup.ts` uses `ensureGotrueUser` as the sole identity path and writes no local `public.users` row (note `schema.users` = `auth.users`); add an assertion/comment.
- [ ] T027 [US3] Run `pnpm --filter @supastack/api test` (org-store unit + existing org/setup tests green); on a clean control plane, run a fresh setup and confirm quickstart §3 (operator in `auth.users`, org owned, listed by `/platform/organizations`; setup re-POST → 410, atomic rollback on forced failure).
...
- [ ] T028 [P] [US4] `/v1` contract no-drift: run `pnpm --filter @supastack/api test` management-api + contract suites green (Constitution IV; no OpenAPI snapshot change).
- [ ] T029 [P] [US4] CLI/MCP regression on the VM: `tests/cli-e2e/{db-push,migration-list,gen-types,mcp-roundtrip}.sh` pass against `api.<apex>/v1`.
- [ ] T030 [P] [US4] Login smoke: sign in to the studio → dashboard loads (token POST to `/auth/v1/token`; confirm not derived from the changed base).
- [ ] T031 [US4] Integration suite (live VM): `tests/integration/{provision-instance,backup,backup-retention}.test.ts` pass — confirms the `/api/v1` engine (instances + backups) is retained.
- [ ] T032 [US4] Engine-delegation smoke: in the studio, create a project (delegates `/platform/projects` → `/api/v1/instances` → worker) and trigger a restart (→ `/api/v1/instances/:ref/restart`); both succeed.
...
- [ ] T033 [P] Finalize the runbook `docs/changes/086-platform-base-root-url.md` (deploy, rollback, verification results).
- [ ] T034 [P] File follow-up issues: (a) make platform backup/restore + audit routes real (they're stubs — Studio backups page shows nothing, restore no-ops); (b) rehome a browser e2e harness to target the platform studio (replacing the removed apps/web Playwright suite).
- [ ] T035 Update `CLAUDE.md` active-feature status to implemented/deployed and refresh the `project_legacy_studio_retire` memory with the outcome.
...
- **US2 (P2)** and **US3 (P2)** are independent of US1 and of each other (different files: `apps/web/*` vs `apps/api/{services/org-store,routes/setup,routes/platform-misc}`). T024 touches `platform-misc.ts` — settle T002 first.
- **US4 (P3)** runs after the others are in place (it verifies them).

**Recommended order (from plan §Phase 2)**: US3 + US2 first (no deploy dependency) → US1 atomic cutover → US4 verification.
