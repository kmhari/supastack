...
> **Why `GENERATED BY DEFAULT AS IDENTITY`, not a hand-rolled `CREATE SEQUENCE … OWNED BY backups.seq`?** The migration runs as a superuser while the `backups` table is owned by the app role, so `CREATE SEQUENCE … OWNED BY` fails with `sequence must have same owner as table it is linked to` (caught live during the in-place deploy). An identity column's sequence is table-owned + engine-managed (Postgres backfills existing rows on `ADD COLUMN`, no `GRANT USAGE` needed), so it works regardless of the connecting role.
