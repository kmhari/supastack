...
**Cross-refs**: GitHub issue [kmhari/supastack#2](https://github.com/kmhari/supastack/issues/2)

## Clarifications

### Session 2026-05-23

- Q: Should supastack use manual DNS-01 (operator adds TXT records themselves) or automated Cloudflare API (supastack creates TXT records automatically)? → A: Manual DNS-01 — wizard shows TXT record values; operator adds them at their registrar; works with any DNS provider. Cloudflare API automation tracked separately in issue #6.
...
- Q: How should cert/key files be shared between the API and Caddy containers? → A: New `certs-data` Docker volume mounted at `/var/supastack/certs` in both `api` (write) and `caddy` (read).
- Q: Should the "Issue Certificate" button be gated on DNS confirmation? → A: Yes — button is disabled (greyed out) until both TXT records show ✅ in the live DNS check; auto-poll every 10s makes it appear naturally once DNS propagates.

## User Scenarios & Testing *(mandatory)*

### User Story 1 — Issue a wildcard certificate during the first-time setup wizard (Priority: P1)

A new operator stands up a fresh supastack host, points their domain at it, and walks the `/setup` wizard. Today, the wizard collects the apex domain, verifies DNS, and tells the operator certificates will be issued on-demand for each subdomain. The operator clicks Continue and starts using the platform — the first time anyone hits `<ref>.<apex>` or `studio-<ref>.<apex>` after a new project is created, they pay a 3-5 second ACME handshake penalty, and at scale Let's Encrypt's per-subdomain rate limits become a real concern.
...
**Why this priority**: The /setup wizard is the only path into supastack for a new operator. If this change accidentally introduces a step ordering issue, a validation regression, or any other speed-bump in signup, the platform becomes harder to adopt — directly counter to the goal of the feature.
...
1. **Given** a wiped host where the control-plane database has no `setup_state.completed_at` timestamp, **When** an operator navigates to any supastack URL, **Then** they are sent to `/setup`.
...
4. **Given** an operator on the DNS-01 certificate step, **When** they add the TXT records at their registrar, **Then** the wizard auto-polls DNS every 10 seconds and shows ⏳/✅ per record; once both records show ✅ the "Issue Certificate" button becomes enabled; clicking it walks through "Completing ACME challenge → Certificate issued" with each step succeeding or showing a clear error.
...
- **FR-001**: Supastack MUST extend the existing `/setup` wizard with a new step for wildcard certificate issuance. The step appears after apex-DNS A-record verification and before /setup is marked complete. The step is skippable — the operator may proceed without a wildcard certificate.
...
- **FR-003**: The wizard MUST auto-poll DNS (public resolvers 1.1.1.1, 8.8.8.8) every 10 seconds and display a per-record ✅/⏳ status for each TXT challenge value. The "Issue Certificate" button MUST remain disabled until both TXT values are confirmed visible. Once both show ✅, the button enables. Clicking it triggers the ACME challenge completion — the platform MUST NOT attempt the ACME challenge while any record is still pending.
...
- **FR-013**: When the wildcard certificate is within 30 days of expiry, supastack MUST display a dashboard-visible alert listing the expiry date and a direct link to the renewal flow. The alert MUST remain visible until the operator either renews the certificate or explicitly dismisses it.
...
- **FR-015**: Supastack MUST log every certificate issuance and renewal event (start, finish, outcome, error if any) to its existing audit log so the operator has a verifiable history of TLS state transitions.

#### Verification Workflow

- **FR-016**: To enable end-to-end testing of the new /setup flow, the implementation MUST be deployable to a wiped host where the entire supastack stack has been torn down and re-installed from scratch. After the reset, the operator MUST be able to walk the full signup → /setup → DNS-01 wildcard-issuance flow and end at a working dashboard.
...
- **SC-002**: After the wildcard is issued, the first HTTPS request to any newly-created per-instance subdomain (`<ref>.<apex>`, `studio-<ref>.<apex>`, `api.<apex>`, or any future supastack-managed subdomain) returns the response body in under 500 milliseconds end-to-end — no per-subdomain ACME handshake penalty.
...
- **SC-007**: Operators who skip the DNS-01 step entirely end up on a working deployment using the existing on-demand TLS path. Supastack remains fully usable without a wildcard; the wildcard is an opt-in optimization.
...
- "Reset the VM for re-setup testing" means: stop all supastack containers, drop the control-plane Postgres data volume, wipe `/var/supastack/instances/` and `/var/supastack/certs/`, and remove any remaining Docker volumes. The host's outer DNS configuration (apex A record pointing at the VM IP) stays in place. This is a one-time pre-release validation activity.
