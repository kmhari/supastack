...
### User Story 3 — Operator restricts a CLI invocation to read-only data access (Priority: P2)
...
- **FR-002**: Selfbase MUST also expose the matching delete endpoint (`DELETE /v1/projects/{ref}/cli/login-roles`) which invalidates the password of the calling project's CLI roles immediately (by setting `VALID UNTIL` to the past, or equivalent), so an operator who wants to lock out CLI access mid-window can do so via the CLI's own DELETE call without waiting for the 5-minute timer.
...
- **FR-004**: When the client requests `read_only: true`, the role returned MUST be granted `IN ROLE supabase_read_only_user` (a role that already exists on every per-instance Postgres because it is shipped by the upstream `supabase/postgres` image). The upstream CLI's auto-`SET SESSION ROLE` post-connect handler MUST cause the connection to operate with read-only privileges. Any write attempted through that connection MUST be refused by Postgres with a permission error.
...
- **SC-007**: The feature does not regress any currently shipped `supabase migration *` subcommand's behaviour. Specifically: `migration list/new/up/down/repair/squash/fetch` all continue to work in both the legacy `--password` flow AND the new password-less flow, with the wire-level behaviour against the control plane API unchanged in the legacy flow.
...
- The CLI's `supabase link` command will continue to require the operator to either supply `--password` or have the new endpoint available — this feature does not address the `supabase link` UX separately; the same endpoint that unblocks `db push` unblocks `link` as a side effect, because `link` uses the same `NewDbConfigWithPassword` resolution path.
...
- Selfbase's top-level Supavisor pooler (`pooler.<apex>:6543`) authenticates against the per-project Postgres on demand, so a freshly rotated password on a `cli_login_*` role becomes usable through the pooler without explicit cache invalidation. The CLI's existing backoff/retry loop in `initPoolerLogin` handles any sub-second propagation lag.
