...
**Target Platform**: Supastack control-plane stack (api / worker / web), per-instance auth container is `supastack-<ref>-auth` (gotrue). All changes ship via the standard rsync → `docker compose build` → `docker compose up -d` cycle on the production VM (`ubuntu@148.113.1.164`, apex `supaviser.dev`).
...
- `supabase` CLI must keep working unchanged — the `_supastack` extension key on GET must be additive-only.
...
├── contracts/
│   ├── auth-config-get-response.md       ← GET shape including _supastack extension
...
apps/api/src/services/
  ...
  runtime-config-store.ts                MODIFIED — getConfig() composes the _supastack extension into the GET response
...
apps/api/tests/unit/
  ...
  auth-config-response-shape.test.ts     NEW — GET includes _supastack.fieldStatus; CLI-ignorance simulated
...
The `_supastack` key is namespaced so unmodified upstream `supabase` CLI consumers ignore it. The field-status map is computed once at module load time from `AUTH_CONFIG_FIELD_STATUS` and reused across requests (zero per-request cost).

The PATCH handler is unchanged — `_supastack` is read-only / response-only.
...
Pseudo-flow:
```
1. Provision (or reuse) a test project on $SUPASTACK_APEX
2. Load assertions library; iterate AUTH_CONFIG_HONORED (read via `curl GET ...config/auth | jq '._supastack.fieldStatus | to_entries[] | select(.value.status=="honored").key'`)
3. For each field:
...
- How to read the `_supastack.fieldStatus` in API responses for SREs
...
1. **GoTrue env-var names for the 17 OAuth providers + Slack OIDC variant + mailer extras + rate limits + sessions.** Source: upstream `gotrue` repo's `internal/conf/configuration.go` (env-var struct tags). Need to confirm names match what supastack's pinned image (current `supabase/gotrue:vX.Y.Z`) actually reads. Risk: a couple of older `mailer_notifications_*` fields may have GoTrue support gaps that force them into `stored_only` instead of `honored`. Output: the authoritative list of ~141 (env → supastack field) mappings.
...
- GET response shape with `_supastack` extension (JSON example)
- No new DB entities

### `contracts/auth-config-get-response.md`

Specifies the augmented GET response shape. JSON example showing every classification kind. CLI-compat statement (unmodified `supabase` ignores `_supastack`).
...
5. GET auth-config via CLI → confirm `_supastack.fieldStatus` shape
