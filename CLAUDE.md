...
<!-- SPECKIT START -->
**Active feature plan**: feature 084 — GoTrue Control-Plane Auth + Multi-Tenant Orgs + Cloud RBAC. Plan: [specs/084-gotrue-control-plane-auth/plan.md](specs/084-gotrue-control-plane-auth/plan.md). Spec: [specs/084-gotrue-control-plane-auth/spec.md](specs/084-gotrue-control-plane-auth/spec.md). Replaces the hand-rolled human auth (`sb_sid` Redis session + `studio-gotrue.ts` JWT shim + bespoke `users` table) with a real **GoTrue** control-plane service (`supabase/gotrue:v2.186.0`, `GOTRUE_JWT_SECRET = HKDF(master key)` — no new secret). Adds multi-tenant **organizations** (Owner/Administrator/Developer/Read-only RBAC, org-scoped `authorize(req, action, orgId)`), member management + **email invites**, and SMTP. **Splits** the conflated `org` singleton into an `installation` singleton (apex + backups) + a multi-row `organizations` table; every project gets `organization_id`. Token model A (GoTrue tokens everywhere). Greenfield cutover (drops `users`/`org`/`invites`). PATs + OAuth 2.1 MCP unchanged (re-resolve against `auth.users`). Studio (IS_PLATFORM) supplies the login + org/member UI; backend matches its shapes. Out of scope: MFA, social login, billing. Built on feature 025 (Studio-as-dashboard).
