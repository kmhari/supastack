...
<!-- SPECKIT START -->
**Active feature plan**: feature 024 — Secret Reveal: no-password UI masking (issue #73). Plan: [specs/024-secret-reveal-no-password/plan.md](specs/024-secret-reveal-no-password/plan.md). Spec: [specs/024-secret-reveal-no-password/spec.md](specs/024-secret-reveal-no-password/spec.md). Removes the password re-auth gate from all three secret-reveal surfaces: JWT keys page, API keys page, and all 6 OAuth provider drawer forms. Backend drops password verification from `POST /instances/:ref/credentials/reveal` and adds a new `GET /projects/:ref/config/auth/reveal` endpoint for OAuth secrets (plaintext auth config). Frontend removes RevealDialog, simplifies useRevealCredentials hook, updates JWT/API key pages to one-way reveal (button becomes Copy), and enables Reveal in all 6 OAuth forms (button disappears after reveal). Audit log entries retained on all surfaces.
