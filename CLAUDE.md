...
<!-- SPECKIT START -->
**Active feature plan**: feature 084 — Migrate UI Primitives to Supabase Design System. Plan: [specs/084-migrate-ui-primitives/plan.md](specs/084-migrate-ui-primitives/plan.md). Spec: [specs/084-migrate-ui-primitives/spec.md](specs/084-migrate-ui-primitives/spec.md). Replaces supastack's 18 local shadcn UI primitives in `apps/web/src/components/ui/` with vendored copies from Supabase `packages/ui`; migrates CSS token system from shadcn conventions to Supabase semantic tokens; updates all ~136 call sites (Button: `variant=`→`type=`, size renames; others: mostly drop-in). 5 USs: US1 token system, US2 Button migration, US3 shadcn bulk, US4 excluded components, US5 e2e verification.
