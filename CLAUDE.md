...
<!-- SPECKIT START -->
**Active feature plan**: feature 025 — Shared Studio (IS_PLATFORM=true). Plan: [specs/025-shared-studio-platform/plan.md](specs/025-shared-studio-platform/plan.md). Spec: [specs/025-shared-studio-platform/spec.md](specs/025-shared-studio-platform/spec.md). Replaces N per-project Studio containers with a single shared Studio in the control plane at `IS_PLATFORM=true`, served at the apex root (`https://<apex>/`). Adds platform-proxy routes in `apps/api` that resolve `ref → portKong` and forward to pg-meta, storage, auth admin, and analytics on the per-instance Kong gateway. Removes `studio` from the per-project compose template. Zero Studio source changes — all behaviour driven by `NEXT_PUBLIC_IS_PLATFORM=true` + `NEXT_PUBLIC_API_URL` env vars.
