...
4. **Step 3** — Enter apex domain (e.g. `supastack.example.com`) → confirm A record DNS resolves → Continue.
...
4. Run `pnpm test:cli` (requires `SUPASTACK_APEX`, `SUPASTACK_PAT`, `SUPASTACK_PROJECT_REF` env vars).
