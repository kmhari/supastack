...
**Follow-up**: [issue #21](https://github.com/kmhari/supastack/issues/21) — close the shape-vs-behavioral parity gap for stored-only auth-config fields
...
**CLI compatibility note**: the original spec assumed `supabase postgres-config update --max-rows N` and `supabase config update --auth-*` worked. CLI v2.72+ removed those imperative flags in favor of declarative `supabase config push` reading `config.toml`. `config push` needs 3 additional endpoints supastack doesn't yet provide (`billing/addons`, `config/database/postgres`, `ssl-enforcement`). That work is tracked as [issue #26](https://github.com/kmhari/supastack/issues/26). The HTTP endpoints in this feature are the stable contract — they work today via curl and will work via `config push` once issue #26 lands.
...
⚠ The per-instance template has every `GOTRUE_EXTERNAL_*` provider line **commented out by default**. The mapper marks them honored (the env var name is the contract), but until the template is updated or operators uncomment manually, setting `external_<provider>_enabled: true` writes the env line but has no runtime effect. Tracked in [#21](https://github.com/kmhari/supastack/issues/21).

Stored-only today: hook URIs and secrets (Cloud-only serverless hooks supastack doesn't ship), SAML SSO (no SAML provisioning yet), `security_captcha_*` (operators bring their own keys), various MFA + rate-limit + session knobs not wired into the template. Full per-field inventory is the deliverable of issue #21.
...
- **Secrets at rest** — the snapshot JSONB is encrypted via the master-key envelope (`@supastack/crypto`), same as `projectSecrets.encryptedValue`. Plaintext secrets never appear in the snapshot column.
...
- CLI e2e: **deferred** — see [issue #26](https://github.com/kmhari/supastack/issues/26). The upstream `supabase` CLI evolved away from imperative flags between v2.41 and v2.72; full `supabase config push` compat requires 3 additional endpoints out of scope for this feature.
