...
1. **Backend** — promotes 144 auth-config fields from "silently inert" to "actually wired into the per-instance GoTrue container". Honored-field count climbs from 24 to 169 (out of 234 total). `GET /v1/projects/<ref>/config/auth` now surfaces a `_supastack.fieldStatus` extension that tells operators per-field whether a value will take effect.
2. **Dashboard** — new page at `Auth → Providers` mirroring [supabase.com/dashboard/.../auth/providers](https://supabase.com/dashboard). 21 OAuth provider drawers + Email/Phone toggle rows + 3 disabled "Coming soon" placeholders for things supastack doesn't ship yet.
...
| SAML 2.0                          | Disabled, "Coming soon" → [#61](https://github.com/kmhari/supastack/issues/61) |
| Web3 Wallet                       | Disabled, "Coming soon" → [#72](https://github.com/kmhari/supastack/issues/72) |
...
**The Reveal button** on Client Secret is disabled. To update a saved secret, paste a new value and Save — leaving the field blank keeps the previous value. Plaintext readback of an existing secret is tracked in [#73](https://github.com/kmhari/supastack/issues/73).
...
## SRE / CLI: reading `_supastack.fieldStatus`
...
curl -sS -H "Authorization: Bearer $PAT" \
  ...
  jq '._supastack.fieldStatus | to_entries[:5]'
```
...
- **`unsupported`** — supastack has deliberately chosen not to honor (Cloud-only OAuth server fields; see #63).
...
curl -sS -H "Authorization: Bearer $PAT" \
  "https://<apex>/v1/projects/<ref>/config/auth" | \
  jq '._supastack.fieldStatus | [.[] | .status] | group_by(.) | map({status: .[0], count: length})'
...
CLI users (`supabase config get --auth`) work unchanged — the `_supastack` key is silently ignored.
...
```bash
ssh ubuntu@<vm-host> "sudo docker logs supastack-<ref>-auth --tail 50"
```
...
curl -sS -H "Authorization: Bearer $PAT" \
  "https://<apex>/v1/projects/<ref>/config/auth" | \
  jq '._supastack.fieldStatus.external_discord_enabled'
...
- SMS provider configuration UI (Twilio/MessageBird/Textlocal/Vonage) — tracked in [#66](https://github.com/kmhari/supastack/issues/66) (backend) + [#68](https://github.com/kmhari/supastack/issues/68) (dashboard)
- Mailer template editor — backend promotes the 37 mailer fields here; dashboard page tracked in [#71](https://github.com/kmhari/supastack/issues/71)
- MFA configuration UI — tracked in [#65](https://github.com/kmhari/supastack/issues/65) (gated on a GoTrue image bump)
- Auth hooks (`hook_*`) — tracked in [#64](https://github.com/kmhari/supastack/issues/64)
- Captcha env wiring — tracked in [#62](https://github.com/kmhari/supastack/issues/62)
- SAML SSO support — tracked in [#61](https://github.com/kmhari/supastack/issues/61)
- Web3 Wallet sign-in — tracked in [#72](https://github.com/kmhari/supastack/issues/72)
- Custom OAuth server (supastack as IdP) — tracked in [#63](https://github.com/kmhari/supastack/issues/63), recommended to stay `unsupported`
- Migrating OAuth provider secrets into `vault.secrets` — tracked in [#70](https://github.com/kmhari/supastack/issues/70)
- Plaintext reveal of an existing OAuth secret from the dashboard — tracked in [#73](https://github.com/kmhari/supastack/issues/73)
...
- **Drift guard**: `apps/api/tests/contract/upstream-auth-config-snapshot.test.ts` fails the build if the upstream OpenAPI snapshot adds a field supastack hasn't classified.
...
3. Verify the `_supastack.fieldStatus` JSON shape via curl (Smoke 3)
