...
- **worker**: pooler-reconciler classifier with one fixture pair per drift class (all 7); provision happy path + idempotency + failure modes (missing row, missing apex, health-timeout, auth-class probe failure, vault-enable failure); cert issuance happy/ACME-error/retry; backup, lifecycle, caddy-reload, OAuth cleanup jobs. Mocks placed at the import seam (`@selfbase/docker-control`, `@selfbase/db`, `@selfbase/crypto`, `undici`, `pg`, `bullmq`, `ioredis`, `child_process`) — units under test never mocked.
